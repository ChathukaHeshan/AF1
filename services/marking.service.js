...
const getMarkings = async (request, response) => {
    try {
        const markings = await returnAllMarkingsWithAuthors();
        response.status(200).json({ markings: markings });
    } catch (error) {
        response.status(404).json({ error: error.message });
    }
};

const returnAllMarkingsWithAuthors = () => {
    return  Marking.find().populate('author');
}

const returnAllMarking = () => {
    return  Marking.find();
}
...
const editMarking = async(request, response) => {
    Marking.findByIdAndUpdate(request.params.id, {
        $set: request.body
    }, (error, data) => {
        if (error) {
            console.log(error)
            ...
            response.json(data)
            console.log(' Marking updated successfully !')
        }
    })
}
...
const approveMarking = async(request, response) => {
    if(request.body.approve){
        ...
        Marking.findByIdAndUpdate(request.body.id, {
            isApproved: true
        }, (error, data) => {
            if (error) {
                console.log(error)
        ...
        Marking.findByIdAndDelete(request.body.id, function (err, docs) {
            if (err){
